...
class Dice {
	constructor(room, amount, starter) {
		...
		this.startMessage = '<div class="infobox"><b style="font-size: 14pt; color: #24678d"><center><span style="color: ' + hashColor(starter) + '">' + Chat.escapeHTML(starter) + '</span> has started a game of dice for <span style = "color: green">' + amount + '</span> ' + buck + '!</center></b><br>' +
	...
	join(user, self) {
		let money = Economy.read(user.userid);
		...
		if (this.players.length) {
			//there's only gonna be one user anyway
			//if (this.players[0].latestIp === user.latestIp) return self.errorReply("You have already joined this game of dice under the alt '" + this.players[0].name + "'.");
		}
		this.players.push(user);
		this.room.add('|uhtmlchange|' + this.room.diceCount + '|' + this.startMessage + '<center>' + user.name + ' has joined the game!</center></div>').update();
		console.o
	...
	play() {
		...
		let money1 = Economy.read(p1), money2 = Economy.read(p2);
		if (money1 < this.bet || money2 < this.bet) {
			...
			this.players.splice(this.players.indexOf(user), 1);
			this.room.add('|uhtmlchange|' + this.room.diceCount + '|' + this.startMessage + '<center>' + other.name + ' has joined the game!</center>').update();
			return;
		}
		let players = this.players.map(user => user.name).join(' and ');
		...
		setTimeout(() => {
			...
			this.room.add('|uhtmlchange|' + this.room.diceCount + '|<div class="infobox"><center>' + players + ' have joined the game!<br /><br />' +
				...
				'<b style = "color:' + hashColor(p1) + ';">' + p1.name + '</b> rolled ' + (roll1 + 1) + '!<br />' +
				'<b style = "color:' + hashColor(p2) + ';">' + p2.name + '</b> rolled ' + (roll2 + 1) + '!<br />' +
				'<b style = "color:' + hashColor(winner) + ';">' + winner.name + '</b> has won <b style="color:green">' + this.bet + '</b> ' + buck + '!<br />' +
				'Better luck next time, ' + Chat.escapeHTML(loser.name) + '!'
			).update();
			Economy.write(winner.userid, this.bet);
			Economy.write(loser.userid, -this.bet);
			this.end();
		}, 800);
	}
