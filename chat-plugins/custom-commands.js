...
exports.commands = {
	...
	tell: function (target, room, user, connection, cmd) {
		...
		if (tells) tells.push(date + '<b><font color = "' + Core.color(user.userid) + '">' + user.name + ':</color></b> ' + Tools.escapeHTML(target));
		else {
			tells = [];
			tells.push(date + '<b><font color = "' + Core.color(user.userid) + '">' + user.name + ':</color></b> ' + Tools.escapeHTML(target));
		}
