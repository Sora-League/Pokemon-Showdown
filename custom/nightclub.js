...
function formatHTML (message) {
	/*var spoiler;
	if (message.match(/spoiler:/i) && message.trim() !== 'spoiler:') {
		var parts = message.split('spoiler:');
		message = parts[0] + 'spoiler: ';
		spoiler = '<span class = "spoiler" style = "text-shadow: none;">' + parts[1].trim() + '</span>';
	}*/
...
var cmds = {
	...
	off: function (target, room, user) {
		if (!this.can('mute', room, null)) return this.sendReply('You must be ranked % or higher to toggle this room\'s nightclub mode.');
		if (!nightclubs[room.id]) return this.sendReply('This room\'s isn\'t in nightclub mode yet...');
